"""
Test utilities and fixtures for goto command tests.
"""
import json
import os
import shutil
...
class GotoTestHelper:
    ...
    def create_history(self, history_entries: List[Dict]) -> str:
        """Create a test history file and return its path."""
        with open(self.history_file, 'w') as f:
            json.dump(history_entries, f, indent=2)
        return self.history_file
    
    def run_goto(self, args: List[str], timeout: int = 5) -> Tuple[int, str, str]:
        """
        ...
        """
        cmd = [self.goto_binary] + args
        
        # Add config and history file options if they exist
    ...
    def run_goto_interactive(self, args: List[str], input_text: str = "", timeout: int = 2) -> Tuple[int, str, str]:
        """
...
def goto_helper():
    ...
    if not goto_binary.exists():
        pytest.skip(f"Goto binary not found at {goto_binary}")
